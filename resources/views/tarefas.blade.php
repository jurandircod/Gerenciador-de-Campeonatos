      ...
      <div class="bg-white/90 rounded-xl shadow-lg border border-earth-100 p-6">
        <header class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          <div>
            ...
            <p class="text-sm text-earth-700/80">Cadastre tarefas, defina responsável, área, prioridade e acompanhe o
              status.</p>
          </div>
        ...
        <section class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <form action="{{ route('tarefas.update', $inputs['id']) }}" method="POST" id="formTarefa">
                ...
                <div class="mb-3">
                  ...
                  <textarea name="descricao" rows="3" class="w-full p-2 border rounded mt-1"
                    placeholder="Detalhes (opcional)">{{ $inputs['descricao'] }}</textarea>
                </div>
                  ...
                  <div>
                    ...
                    <select name="area_id" required class="w-full p-2 border rounded mt-1">
                      ...
                      @foreach($areas as $area)
                        <option value="{{ $area->id }}" {{ $inputs['area_id'] == $area->id ? 'selected' : '' }}>{{ $area->nome }}
                        </option>
                      @endforeach
                    </select>
                  </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
                  ...
                  <div>
                    ...
                    <select name="prioridade" class="w-full p-2 border rounded mt-1">
                      <option value="Alta" {{ $inputs['prioridade'] == 'Alta' ? 'selected' : '' }}>Alta</option>
                      <option value="Média" {{ $inputs['prioridade'] == 'Média' ? 'selected' : '' }}>Média</option>
                      <option value="Baixa" {{ $inputs['prioridade'] == 'Baixa' ? 'selected' : '' }}>Baixa</option>
                    </select>
                  </div>
                ...
                <div class="flex justify-end gap-2 mt-4">
                  ...
                  <button type="submit" class="px-4 py-2 bg-earth-500 text-white rounded">Atualizar Tarefa</button>
                </div>
              </form>
            @else
              ...
              <form action="{{ route('tarefas.store') }}" method="POST" id="formTarefa">
                ...
                <div class="mb-3">
                  <label class="block text-sm font-medium text-earth-700">Descrição</label>
                  <textarea name="descricao" rows="3" class="w-full p-2 border rounded mt-1"
                    placeholder="Detalhes (opcional)">{{ old('descricao') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select name="area_id" required class="w-full p-2 border rounded mt-1">
                      <option value="">-- Selecionar --</option>
                      @foreach($areas as $area)
                        <option value="{{ $area->id }}" {{ old('area_id') == $area->id ? 'selected' : '' }}>{{ $area->nome }}
                        </option>
                      @endforeach
                    </select>
                  </div>
                ...
                <div class="flex justify-end gap-2 mt-4">
                  <a href="{{ route('tarefas.index') }}" class="px-4 py-2 border rounded">Cancelar</a>
                  <button type="submit" class="px-4 py-2 bg-earth-500 text-white rounded">Salvar tarefa</button>
                </div>
              </form>
            @endif
          </div>
          ...
          <div class="lg:col-span-2">
                ...
                <tbody id="tasksTableBody">
                  @foreach($tarefas as $tarefa)
                    <tr class="even:bg-earth-50">
                      ...
                      <td class="p-3 text-sm text-earth-700">{{ \Illuminate\Support\Str::limit($tarefa->descricao, 120) }}
                      </td>
                      <td class="p-3">{{ $tarefa->responsavel->nome ?? '—' }}</td>
                      <td class="p-3">{{ $tarefa->area->nome ?? '—' }}</td>
                      <td class="p-3 text-sm">{{ $tarefa->prazo }}</td>
                      <td class="p-3"><span
                          class="px-2 py-1 rounded text-xs font-semibold {{ $tarefa->prioridade == 'Alta' ? 'bg-earth-300' : ($tarefa->prioridade == 'Média' ? 'bg-earth-200' : 'bg-earth-50') }} text-earth-800">{{ $tarefa->prioridade }}</span>
                      </td>
                      <td class="p-3">
                        <form action="{{ route('tarefas.update', $tarefa->id) }}" method="POST" class="inline-block">
                          ...
                          <select name="status" onchange="this.form.submit()"
                            ...
                            <option value="Em Andamento" {{ $tarefa->status == 'Em Andamento' ? 'selected' : '' }}>Em
                              Andamento</option>
                            <option value="Concluída" {{ $tarefa->status == 'Concluída' ? 'selected' : '' }}>Concluída
                            </option>
                          </select>
                        </form>
                      </td>
            ...
            <div class="mt-3 text-sm text-earth-700">Total: <strong>{{ $tarefas->count() }}</strong> tarefas</div>
          </div>
        </section>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      function applyFilter() {
        ...
        Array.from(tbody.querySelectorAll('tr')).forEach(tr => {
          ...
          const status = (tr.children[6].querySelector('select') ? tr.children[6].querySelector('select').value.toLowerCase() : '');

          const match = (!q || titulo.includes(q) || desc.includes(q) || resp.includes(q) || area.includes(q))
            && (!a || area.includes(a))
            && (!r || resp.includes(r))
            && (!s || status.includes(s));
      ...
      clearBtn.addEventListener('click', () => { search.value = ''; filterArea.value = ''; filterResp.value = ''; filterStatus.value = ''; applyFilter(); });

      // export CSV
      document.getElementById('exportCsv').addEventListener('click', () => {
        const headers = ['Título', 'Descrição', 'Responsável', 'Área', 'Prazo', 'Prioridade', 'Status'];
        const rows = [];
        Array.from(tbody.querySelectorAll('tr')).forEach(tr => {
          ...
          const status = tr.children[6].querySelector('select') ? tr.children[6].querySelector('select').value.trim() : '';
          rows.push([titulo, desc, resp, area, prazo, prioridade, status]);
        });
        const csv = [headers.join(','), ...rows.map(r => r.map(c => `"${String(c).replace(/\"/g, '""')}"`).join(','))].join('\n');
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'tarefas.csv'; link.click();
      });
    });
  </script>
@endpush