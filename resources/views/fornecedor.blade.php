    ...
    <div class="bg-white/90 rounded-xl shadow-lg border border-earth-100 p-6">
      <header class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        ...
        <div class="flex gap-2">
          ...
          <a href="{{ route('fornecedores.store') }}" class="px-4 py-2 border rounded text-earth-700">Novo fornecedor</a>
        </div>
      </header>
        ...
        <div class="lg:col-span-1 bg-white p-4 rounded shadow-sm border border-earth-100">
          ...
          <form action="{{ route('fornecedores.store') }}" method="POST" id="formForn">
            ...
            <div class="mb-3">
              ...
              <input name="nome" type="text" required class="w-full p-2 border rounded mt-1" placeholder="Nome da empresa / pessoa" value="{{ old('nome') }}">
            </div>
            ...
            <div class="mb-3 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input name="contato_nome" type="text" class="w-full p-2 border rounded mt-1" value="{{ old('contato_nome') }}">
              </div>
              <div>
                <label class="block text-sm font-medium text-earth-700">Telefone</label>
                <input name="contato_telefone" type="text" class="w-full p-2 border rounded mt-1" value="{{ old('contato_telefone') }}">
              </div>
            </div>

            <div class="mb-3">
              <label class="block text-sm font-medium text-earth-700">Email</label>
              <input name="contato_email" type="email" class="w-full p-2 border rounded mt-1" value="{{ old('contato_email') }}">
            </div>

            <div class="mb-3 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label class="block text-sm font-medium text-earth-700">Valor orçado</label>
                <input name="valor_orcamento" type="number" step="0.01" class="w-full p-2 border rounded mt-1" value="{{ old('valor_orcamento') }}" placeholder="0.00">
              </div>
              <div>
            ...
            <div class="mb-3">
              ...
              <textarea name="observacoes" rows="3" class="w-full p-2 border rounded mt-1" placeholder="Notas sobre orçamento/contrato">{{ old('observacoes') }}</textarea>
            </div>

            <div class="flex justify-end gap-2 mt-4">
              <a href="{{ route('fornecedores.index') }}" class="px-4 py-2 border rounded">Cancelar</a>
        ...
        <div class="lg:col-span-2">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div class="flex-1 flex gap-2">
              <input id="searchForn" placeholder="Pesquisar por nome, contato, telefone ou observação" class="flex-1 p-2 border rounded" />
            ...
            <table class="w-full min-w-[1000px]">
              ...
              <tbody id="fornTableBody">
                ...
                <tr class="even:bg-earth-50">
                  ...
                  <td class="p-3 text-sm">{{ $f->contato_nome }}<br><span class="text-xs text-earth-700">{{ $f->contato_email }}</span></td>
                  <td class="p-3">{{ $f->contato_telefone }}</td>
                  <td class="p-3">R$ {{ number_format($f->valor_orcamento ?? 0, 2, ',', '.') }}</td>
                  <td class="p-3">{{ $f->status }}</td>
                  <td class="p-3 text-sm text-earth-700">{{ \Illuminate\Support\Str::limit($f->observacoes, 150) }}</td>
                  <td class="p-3 text-center">
                    <a href="{{ route('fornecedores.edit', $f) }}" class="px-3 py-1 border rounded text-sm">Editar</a>

                    <form action="{{ route('fornecedores.destroy', $f) }}" method="POST" class="inline-block ml-2" onsubmit="return confirm('Excluir esse fornecedor?');">
                      @csrf
                      @method('DELETE')
          ...
          <div class="mt-3 text-sm text-earth-700">Total: <strong>{{ $fornecedores->count() }}</strong> fornecedores</div>
        </div>
      </section>
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    function applyFilter(){
      ...
      Array.from(tbody.querySelectorAll('tr')).forEach(tr => {
        ...
        const match = (!q || nome.includes(q) || tipo.includes(q) || contato.includes(q) || tel.includes(q) || valor.includes(q) || obs.includes(q))
          && (!t || tipo.includes(t))
          && (!s || status.includes(s));
    ...
    clearBtn.addEventListener('click', ()=>{ search.value=''; filterTipo.value=''; filterStatus.value=''; applyFilter(); });

    // export CSV
    document.getElementById('exportCsv').addEventListener('click', ()=>{
      const headers = ['Nome','Tipo','ContatoNome','ContatoEmail','Telefone','Valor','Status','Observacoes'];
      const rows = [];
      Array.from(tbody.querySelectorAll('tr')).forEach(tr => {
        if(tr.style.display === 'none') return;
        const nome = tr.children[0].textContent.trim();
        const tipo = tr.children[1].textContent.trim();
        const contatoNome = tr.children[2].childNodes[0] ? tr.children[2].childNodes[0].textContent.trim() : '';
        const contatoEmail = tr.children[2].querySelector('span') ? tr.children[2].querySelector('span').textContent.trim() : '';
        ...
        rows.push([nome,tipo,contatoNome,contatoEmail,telefone,valor,status,obs]);
      });
      const csv = [headers.join(','), ...rows.map(r=>r.map(c=>`"${String(c).replace(/\"/g,'""')}"`).join(','))].join('\n');
      const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
      const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'fornecedores.csv'; link.click();
    });
  });
</script>
@endpush
