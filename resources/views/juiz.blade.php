    ...
    <div class="bg-white/90 rounded-xl shadow-lg border border-earth-100 p-6">
      <header class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        ...
        <div class="flex gap-2">
          ...
          <a href="{{ route('juizes.store') }}" class="px-4 py-2 border rounded text-earth-700">Novo juiz</a>
        </div>
      </header>
        ...
        <div class="lg:col-span-1 bg-white p-4 rounded shadow-sm border border-earth-100">
          ...
          <form action="{{ route('juizes.store') }}" method="POST" id="formJuiz">
            ...
            <div class="mb-3">
              ...
              <input name="nome" type="text" required class="w-full p-2 border rounded mt-1" placeholder="Nome completo" value="{{ old('nome') }}">
            </div>

            <div class="mb-3">
              <label class="block text-sm font-medium text-earth-700">Email</label>
              <input name="email" type="email" required class="w-full p-2 border rounded mt-1" placeholder="email@exemplo.com" value="{{ old('email') }}">
            </div>

            <div class="mb-3 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label class="block text-sm font-medium text-earth-700">Telefone</label>
                <input name="telefone" type="text" class="w-full p-2 border rounded mt-1" value="{{ old('telefone') }}">
              </div>
              <div>
            ...
            <div class="mb-3">
              ...
              <textarea name="experiencia" rows="3" class="w-full p-2 border rounded mt-1" placeholder="Breve descrição de experiência">{{ old('experiencia') }}</textarea>
            </div>

            <div class="flex justify-end gap-2 mt-4">
              <a href="{{ route('juizes.index') }}" class="px-4 py-2 border rounded">Cancelar</a>
              <button type="submit" class="px-4 py-2 bg-earth-500 text-white rounded">Salvar juiz</button>
            </div>
          </form>
        </div>
        ...
        <div class="lg:col-span-2">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div class="flex-1 flex gap-2">
              <input id="searchJuiz" placeholder="Pesquisar por nome, email ou experiência" class="flex-1 p-2 border rounded" />
            ...
            <table class="w-full min-w-[900px]">
              ...
              <tbody id="juizTableBody">
                ...
                <tr class="even:bg-earth-50">
                  ...
                  <td class="p-3 text-sm text-earth-700">{{ \Illuminate\Support\Str::limit($j->experiencia, 150) }}</td>
                  <td class="p-3">{{ $j->status }}</td>
                  <td class="p-3 text-center">
                    <a href="{{ route('juizes.edit', $j) }}" class="px-3 py-1 border rounded text-sm">Editar</a>

                    <form action="{{ route('juizes.destroy', $j) }}" method="POST" class="inline-block ml-2" onsubmit="return confirm('Excluir esse juiz?');">
                      @csrf
                      @method('DELETE')
          ...
          <div class="mt-3 text-sm text-earth-700">Total: <strong>{{ $juizes->count() }}</strong> juízes</div>
        </div>
      </section>
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    function applyFilter(){
      ...
      Array.from(tbody.querySelectorAll('tr')).forEach(tr => {
        ...
        const match = (!q || nome.includes(q) || email.includes(q) || tel.includes(q) || exp.includes(q)) && (!s || status.includes(s));
    ...
    document.getElementById('exportCsv').addEventListener('click', ()=>{
      ...
      const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'juizes.csv'; link.click();
    });
  });
</script>
@endpush
