        ...
        <section class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div class="lg:col-span-1 bg-white p-4 rounded shadow-sm border border-earth-100">
            @if(isset($inputs))
              ...
              <form action="{{ route('orcamentos.update') }}" method="POST" id="formOrc">
                ...
                <div class="mb-3">
                  ...
                  <input name="categoria" type="text" required class="w-full p-2 border rounded mt-1"
                    placeholder="Ex: Patrocínios, Pista, Som" value="{{ $inputs['categoria'] }}">
                </div>

                <div class="mb-3">
                  <label class="block text-sm font-medium text-earth-700">Descrição</label>
                  <input name="descricao" type="text" required class="w-full p-2 border rounded mt-1"
                    placeholder="Descrição do item" value="{{ $inputs['descricao'] }}">
                </div>
                ...
                <div class="mb-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input name="valor_previsto" type="number" step="0.01" required class="w-full p-2 border rounded mt-1"
                      value="{{ $inputs['valor_previsto'] }}">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-earth-700">Valor realizado</label>
                    <input name="valor_realizado" type="number" step="0.01" class="w-full p-2 border rounded mt-1"
                      value="{{ $inputs['valor_realizado'] }}">
                  </div>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <select name="tipo" required class="w-full p-2 border rounded mt-1">
                    <option value="{{ $inputs['tipo'] }}">{{ $inputs['tipo'] }}</option>
            ...
            @else
              ...
              <form action="{{ route('orcamentos.store') }}" method="POST" id="formOrc">
                ...
                <div class="mb-3">
                  <label class="block text-sm font-medium text-earth-700">Categoria</label>
                  <input name="categoria" type="text" required class="w-full p-2 border rounded mt-1"
                    placeholder="Ex: Patrocínios, Pista, Som" value="{{ old('categoria') }}">
                </div>

                <div class="mb-3">
                  <label class="block text-sm font-medium text-earth-700">Descrição</label>
                  <input name="descricao" type="text" required class="w-full p-2 border rounded mt-1"
                    placeholder="Descrição do item" value="{{ old('descricao') }}">
                </div>
                ...
                <div class="mb-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <label class="block text-sm font-medium text-earth-700">Valor previsto</label>
                    <input name="valor_previsto" type="number" step="0.01" required class="w-full p-2 border rounded mt-1"
                      value="{{ old('valor_previsto', 0) }}">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-earth-700">Valor realizado</label>
                    <input name="valor_realizado" type="number" step="0.01" class="w-full p-2 border rounded mt-1"
                      value="{{ old('valor_realizado', 0) }}">
                  </div>
                </div>
          ...
          <div class="lg:col-span-2">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
              <div class="flex-1 flex gap-2">
                <input id="searchOrc" placeholder="Pesquisar por categoria ou descrição"
                ...
                <tbody id="orcTableBody">
                  @foreach($orcamentos as $o)
                    <tr class="even:bg-earth-50">
                      ...
                      <td class="p-3" data-value="{{ $o->valor_previsto }}">R$
                        {{ number_format($o->valor_previsto, 2, ',', '.') }}</td>
                      <td class="p-3" data-value="{{ $o->valor_realizado }}">R$
                        {{ number_format($o->valor_realizado, 2, ',', '.') }}</td>
                      <td class="p-3">{{ $o->tipo }}</td>
                      <td class="p-3 text-center">
                        <a href="{{ route('orcamentos.edit', $o->id) }}" class="px-3 py-1 border rounded text-sm">Editar</a>

                        <form action="{{ route('orcamentos.destroy', $o->id) }}" method="POST" class="inline-block ml-2"
                          onsubmit="return confirm('Excluir esse item orçamentário?');">
                          @csrf
            ...
            <div class="mt-3 text-sm text-earth-700">Total de lançamentos: <strong>{{ $orcamentos->count() }}</strong>
            </div>
          </div>
        </section>
